...
public class Picture extends SimplePicture {
	...
	public void encode(Picture messagePict) {
		...
		for (int row = 0; row < this.getHeight(); row++) {
			for (int col = 0; col < this.getWidth(); col++) {
				...
				while (currPixel.getBlue() % 10 != 0) {
					currPixel.setBlue(currPixel.getBlue() + 1);
				}
				while (currPixel.getRed() % 10 != 0) {
					currPixel.setRed(currPixel.getRed() + 1);
				}
				while (currPixel.getGreen() % 10 != 0) {
	...
	public Picture decode() {
		...
		for (int row = 0; row < this.getHeight(); row++) {
			for (int col = 0; col < this.getWidth(); col++) {
				...
				messagePixel = messagePixels[row][col];
				if((currPixel.getBlue()+currPixel.getRed()+currPixel.getGreen())%10 == 3)
	...
	// add one-half of the difference between the current pixel and pixel
	// located immediately above and to the left of the current pixel.
	// Limit the computed pixel value to 255 and 0 (i.e. if the pixel is >255
	// set it to 255 and if it’s <0 set it to 0).
	...
	public void sharpen(int x, int y, int w, int h) {
		...
		for (int row = y; row < h; row++) {
			for (int col = x; col < w; col++) {
				pixels[row][col].setBlue(Math.abs((pixels[row - 1][col - 1].getBlue() - pixels[row][col].getBlue()) / 2)
						+ pixels[row][col].getBlue());
				pixels[row][col]
						.setGreen(Math.abs((pixels[row - 1][col - 1].getGreen() - pixels[row][col].getGreen()) / 2)
								+ pixels[row][col].getGreen());
				pixels[row][col].setRed(Math.abs((pixels[row - 1][col - 1].getRed() - pixels[row][col].getRed()) / 2)
	...
	public void mirrorArms() // snowman
	{
		int mirrorPoint = 193;
		Pixel topPixel = null;
		Pixel bottomPixel = null;
	...
	public void mirrorGull() // seagull
	{
		int mirrorPoint = 345;
		Pixel rightPixel = null;
		Pixel leftPixel = null;
	...
	public void createCollage() {
		Picture flower1 = new Picture("H:\\APCS\\Unit16\\pixLab\\images\\flower1.jpg");
		Picture flower2 = new Picture("H:\\APCS\\Unit16\\pixLab\\images\\flower2.jpg");
		...
		this.mirrorVertical();
		this.write("H:\\APCS\\Unit16\\pixLab\\images\\collage.jpg");
	}

	public void myCollage() {
		Picture girl = new Picture("H:\\APCS\\Unit16\\pixLab\\images\\KatieFancy.jpg");
		Picture flower2 = new Picture("H:\\APCS\\Unit16\\pixLab\\images\\flower2.jpg");
		this.copy(girl, 0, 0);
		this.copy(flower2, 100, 0);
		this.copy(girl, 200, 0);
		Picture flowerNoBlue = new Picture(flower2);
		flowerNoBlue.zeroBlue();
		this.copy(flowerNoBlue, 300, 0);
		this.copy(girl, 400, 0);
